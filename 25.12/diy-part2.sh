...
if [ -f "$CFG_FILE" ]; then
    ...
    sed -i 's/192\.168\.\$((addr_offset++))\.1/10\.0\.\$((addr_offset++))\.1/g' $CFG_FILE
    # 修改默认主机名
...
if [ -f "$DTS_FILE" ]; then
    # 先修改分区节点的名称地址（从 partition@580000 修改为 @600000）
    sed -i 's/partition@580000/partition@600000/g' "$DTS_FILE"
